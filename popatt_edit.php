...
function updateATT(id){
	$("#ans").html('');
	var iremark= $("#iremark"+id).val();
	var ifl= $("#ifl"+id).val();
	var irate3= $("#irate3"+id).val();
	var irate2= $("#irate2"+id).val();
	var irate1_5= $("#irate1_5"+id).val();
	var irate1= $("#irate1"+id).val();
	var inormal= $("#inormal"+id).val();
	var iotout= $("#iotout"+id).val();
	var iotin= $("#iotin"+id).val();
	var iworkhours= $("#iworkhours"+id).val();
	//alert(iworkhours);
...
<body bgcolor="#FFFFFF"   style="padding-top: 0px;">
  ...
  <tr>
    <td >
     <table width="100%"  cellspacing="2" cellpadding="0">
		...
		[<a href="popatt.php?empno=<?=$empno?>&paycode=<?=$paycode?>" target="_blank">Print ATT</a>] <span id="ans"></span></td></tr>
     </table>
...
FROM    tbatt_approve where empno='$empno' and paycode='$paycode'  order by iworkdate asc ";
	...
	while($row0=mssql_fetch_array($re0)){
      ...
      <tr>
       ...
       <td height="20" align="center"><font size="1" face="Tahoma"><?
       
	   echo $row0['iworkdate2'];
	   
	    
			$select_ot = "select ot from  tbot_mng where workdate='".$row0['iworkdate']."' and empno='$empno' and paycode='$paycode'  ";
				$re_ot=mssql_query($select_ot);
				$row_ot=mssql_fetch_array($re_ot);
				if($row_ot['ot']=='yes'){
				echo "<br><font color='red'>ot=".$row_ot['ot']."</font>";
				}
	   ...
	   ?></font></td>
		...
		?></font>
		 ...
		 echo $row0['iattDate2']?> <?=$row0['iattTime2'];
          ...
          <td align="center"><font size="1" face="Tahoma"><input type="text" id="iworkhours<?=$row0['id']?>" value="<?=$row0['iworkhours']?>" class="form-control" onChange="updateATT('<?=$row0['id']?>');"></font></td>
        <td align="center"><font size="1" face="Tahoma"><input type="text" id="iotin<?=$row0['id']?>" value="<?
		 if($row0['iworkhours']!='0'){
			echo $row0['iotintime'];
			 }
		?>" class="form-control" onChange="updateATT('<?=$row0['id']?>');"></font></td>
        <td align="center"><font size="1" face="Tahoma"><input type="text" id="iotout<?=$row0['id']?>" value="<?
		if($row0['iworkhours']!='0'){
			echo $row0['iotouttime'];
			}
		?>" class="form-control" onChange="updateATT('<?=$row0['id']?>');"></font></td>
        <td align="center"><font size="1" face="Tahoma"><input type="text" id="inormal<?=$row0['id']?>" value="<?
        if($row0['iworkhours']!='0'){
		echo $row0['inormal'];
		}
		?>" class="form-control" onChange="updateATT('<?=$row0['id']?>');"></font></td>
        <td width="41" align="center"><font size="1" face="Tahoma"><input type="text" id="irate1<?=$row0['id']?>" value="<?
		 if($row0['iworkhours']!='0'){
		echo $row0['irate1'];
		 }?>" class="form-control" onChange="updateATT('<?=$row0['id']?>');"></font></td>
        <td width="49" align="center"><font size="1" face="Tahoma"><input type="text" id="irate1_5<?=$row0['id']?>" value="<?
		 if($row0['iworkhours']!='0'){
		echo $row0['irate1_5'];
		 }?>" class="form-control" onChange="updateATT('<?=$row0['id']?>');"></font></td>
        <td width="54" align="center"><font size="1" face="Tahoma"><input type="text" id="irate2<?=$row0['id']?>" value="<?
		if($row0['iworkhours']!='0'){
		echo $row0['irate2'];
		}?>" class="form-control" onChange="updateATT('<?=$row0['id']?>');"></font></td>
        <td width="58" align="center"><font size="1" face="Tahoma"><input type="text" id="irate3<?=$row0['id']?>" value="<?
		if($row0['iworkhours']!='0'){
		echo $row0['irate3'];
		}?>" class="form-control" onChange="updateATT('<?=$row0['id']?>');"></font></td>
        <td align="center"><font size="1" face="Tahoma"><input type="text" id="ifl<?=$row0['id']?>" value="<?
		if($row0['iworkhours']!='0'){
		echo $row0['ifl'];
		}?>" class="form-control" onChange="updateATT('<?=$row0['id']?>');"></font></td>
        <td align="center"><font size="1" face="Tahoma"><input type="text" id="iremark<?=$row0['id']?>" value="<?=iconv("tis-620", "utf-8", $row0['iremark'] );?><?
         if($leavetypeid==''){
			 
					 }else{
						// echo $leavename;
						 }
		?>" class="form-control" onChange="updateRemark('<?=$row0['id']?>');"></font></td>
      </tr>
      <?
	}
	}
