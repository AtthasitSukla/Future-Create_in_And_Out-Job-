    ...
    <script>
        ...
        function type_of_business(){
            ...
            var data = google.visualization.arrayToDataTable([
                ['Task', 'Bath'],
                ['Packing',  <?=number_format($total_packing/1000000,2)?>],
                ['Trading',  <?=number_format($total_trading/1000000,2)?>],
                ['Consult',  <?=number_format($total_Consult/1000000,2)?>],
                ['Transportation Service', <?=number_format($total_trans/1000000,2)?>],
                ['Packaging Management',    <?=number_format($total_packaging/1000000,2)?>],
                ['Onsite Packing',    <?=number_format($total_onsite/1000000,2)?>],
                ['Agreement charge',    <?=number_format($total_agreement/1000000,2)?>],
                ['IT',    <?=number_format($total_it/1000000,2)?>],
            ]);
        ...
        function sab_order_type_county(){
            var ctx = document.getElementById('sab_order_type_county').getContext('2d');
            var chart = new Chart(ctx, {
                // The type of chart we want to create
                type: 'pie',
                ...
                options: {
                    ...
                    title: {
                        display: true,
                        text: 'SAB order County <?=$arr_month[$mmonth]?> <?=$yyear?> (Bath)'
                    },
                    tooltips: {
                        // callbacks: {
                        //     // i = 0;
        ...
        function sab_order_type_packaging(){
            var ctx = document.getElementById('sab_order_type_packaging').getContext('2d');
            var chart = new Chart(ctx, {
                // The type of chart we want to create
                type: 'pie',
                ...
                // The data for our dataset
                data: {
                    labels: [ 'Rack/Box','Module' ],
                    datasets: [{
                        backgroundColor: ['rgb(51, 102, 255)','rgb(194, 0, 0)'] ,
                        // borderColor: ['rgb(255, 181, 0)','rgb(20, 165, 0)'],
                        fill: false,
                        data: [
                            <? 
                ...
                options: {
                    ...
                    title: {
                        display: true,
                        text: 'SAB order Packaging <?=$arr_month[$mmonth]?> <?=$yyear?> (Bath)'
                    },
                    tooltips: {
                        // callbacks: {
                        //     // i = 0;
        ...
        function topre_order_type_county(){
            var ctx = document.getElementById('topre_order_type_county').getContext('2d');
            var chart = new Chart(ctx, {
                // The type of chart we want to create
                type: 'pie',
                ...
                data: {
                    labels: [
                        ...
                        while($row_topre_county = mssql_fetch_array($res_topre_county)){
                            ...
                            $sql_issue = "SELECT Issue_No FROM tbsaborder WHERE Production_Controller='$Production_Controller' AND Delivery_order_date_Month='$mmonth' AND Delivery_order_date_year='$yyear'";
                            $res_issue = mssql_query($sql_issue);
                            while($row_issue = mssql_fetch_array($res_issue)){
                                $Issue_No = $row_issue["Issue_No"];
                                include "connect_inv.php";
                                $sql_price = "SELECT amount 
                            ...
                            $data_price .= number_format($total_price_topre_county/1000000,2).",";
                        }
                        ?>
                        ],
                    datasets: [{
                ...
                options: {
                    ...
                    title: {
                        display: true,
                        text: 'Topre order County <?=$arr_month[$mmonth]?> <?=$yyear?> (Bath)'
                    },
                    tooltips: {
                        // callbacks: {
                        //     // i = 0;
        ...
        function topre_order_type_packaging(){
            var ctx = document.getElementById('topre_order_type_packaging').getContext('2d');
            var chart = new Chart(ctx, {
                // The type of chart we want to create
                type: 'pie',
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        data: [
                            ...
                            $sql_topre_module = "SELECT Issue_No FROM tbsaborder 
                            WHERE Number_of_Boxes>1
                            AND Delivery_order_date_Month='$mmonth' 
                            AND Delivery_order_date_year='$yyear' ";
                            ...
                            echo number_format($total_rackbox_price/1000000,2).",".number_format($total_module_price/1000000,2);
                            ?>
                        ],
                ...
                options: {
                    ...
                    title: {
                        display: true,
                        text: 'Topre order Packaging <?=$arr_month[$mmonth]?> <?=$yyear?> (Bath)'
                    },
                    tooltips: {
                        // callbacks: {
                        //     // i = 0;
