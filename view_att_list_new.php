	...
	<script>
		...
		function updateshift(id) {
			...
			$.post("getajax_att.php", {
				...
				.done(function(data) {
					alert('save complete');
					window.location = 'view_att_list2.php?empno=<?= $empno ?>&paycode=<?= $paycode ?>';
								...
								<div class="panel-body">
									...
									if ($status == '') {
										...
										<table width="100%" cellspacing="2" cellpadding="0">
											<tr>
												<td>
													<h4>Emp No : <?= $empno ?> Name : <?= $empname ?> </h4> <?
										...
										<form name="form" method="post" action="view_att_list2.php?status=save">
...
FROM    tbot_parameter where workdate between '" . $startdate . "' and '" . $enddate . "' and site='$site'  order by workdate asc ";
													...
													if ($num0 > 0) {
														...
														while ($row0 = mssql_fetch_array($re0)) {
															...
															//check leave date
															$select_le = "select * from  tbleave_transaction where '" . $row0['workdate'] . "' between CAST(leavestartdate AS DATE) and CAST(leaveenddate AS DATE) and empno='$empno'  ";
														...
														<tr>
																	...
																	<option value="Night" <?
																										if ($sShift == 'Night') {
																											?> selected<?
																																	...
																																	?>>N</option>
																</select><input type="hidden" class="form-control" name="iShift[]" value="<?= $sShift ?>">
															</td>
															<td align="center"><?
																							...
																							and att_real_date='" . $row0['workdate'] . "' and att_status = 'in' ";
																							...
																							$sql_leave = "SELECT *,convert(varchar, leavestartdate, 101)as leavestartdate1, 
																								convert(varchar, leavestartdate, 103)as leavestartdate3
																								from tbleave_transaction 
																							where '" . $row0['workdate'] . "' between leavestartdate and leaveenddate
																									and leavetypeid in('L0009','L0010') 
																							...
																							$work_arr = explode("-", $row0['workdate']);
																							$work_date = $work_arr[2] . "/" . $work_arr[1] . "/" . $work_arr[0];
																							$work_date_save =  $work_arr[1] . "/" . $work_arr[2] . "/" . $work_arr[0];

																							if ($num_leave > 0) {
																								$row_leave = mssql_fetch_array($res_leave);
																								$start_time = $row_leave['start_time'] == "" ? "00:00:00" : date("H:i:s", strtotime($row_leave['start_time']));
																								$start_time = date("H:i:s", strtotime($start_time));
																								//echo $row_leave['start_time'];
																								//2018-03-21 - 0 1 2
																								$attDateTime1 = $work_date_save . " " . $start_time;
																								$attDate1 = $work_date_save;
																								$attTime1 = $start_time;
																								echo  $work_date . " " . $start_time;

																								echo "<BR><font color='blue'>$leavename</font>";
																							}
															...
															<td align="center"><?
																						...
																						convert(varchar, attDateTime, 108)as attDateTime2 , 
																						convert(varchar, attDateTime, 103)as attDateTime3 from tbatt where empno='" . $empno . "' 
																						and att_real_date='" . $row0['workdate'] . "' and att_status = 'out' order by id desc ";
																							$re5 = mssql_query($select5);
																							$num5 = mssql_num_rows($re5);
																							if ($num5 > 0) {
																								$row5 = mssql_fetch_array($re5);
																							...
																							if ($num_leave > 0) {
																								$row_leave = mssql_fetch_array($res_leave);
																								$end_time = $row_leave['end_time'] == "" ? "00:00:00" : date("H:i:s", strtotime($row_leave['end_time']));
																								$end_time = date("H:i:s", strtotime($end_time));
																								//2018-03-21 - 0 1 2
																								$attDateTime2 = $work_date_save . " " . $end_time;
																								$attDate2 = $work_date_save;
																								$attTime2 = $end_time;
																								echo  $work_date . " " . $end_time;
																	...
																	<?
																					...
																					if ($leavetypeid == '') {
																						...
																						?><input type="button" value="Create Leave" onClick="createleave('<?= $rowot['id'] ?>','<?= $empno ?>','<?= $row0['workdate2'] ?>');"><?
																																																											...
																																																											//}
																																																											?><input type="hidden" class="form-control" name="iattDateTime2[]" value="<?= $attDateTime2 ?>">
															</td> <!-- </td เวลา ออกงาน > -->
															...
															<td align="center"><?
																							...
																							if ($row0['work_type'] == 'H Sat , Sun' || $row0['work_type'] == 'H' || $row0['work_type'] == 'H Sat') {
																								...
																								//	echo $total_ot;
																								?>
																	<input type="text" class="form-control" name="iworkhours[]" value="<?= $total_ot ?>">
																<?
																				//	echo (int)$workhours_wo_ot+$total_ot;
																			} else {
																				//  echo (int)$workhours_wo_ot+$total_ot;
																				?>
																	<input type="text" class="form-control" name="iworkhours[]" value="<?= (int) $workhours_wo_ot + $total_ot ?>">
																<?
																			}
															...
															<td align="center"><?
																							//echo $otin;
																							?> <input type="text" name="iotin[]" class="form-control" value="<?= $otin ?>"></td>
															<td align="center"><?
																							//echo $otout;
																							?><input type="text" name="iotout[]" class="form-control" value="<?= $otout ?>"></td>
															<td align="center"><?
																							// echo (int)$workhours_wo_ot;
																							?><input type="text" name="inormal[]" class="form-control" value="<?= (int) $workhours_wo_ot ?>"></td>
															<td width="84" align="center"><?
																										...
																										?><input type="text" name="irate1[]" class="form-control" value="<?
																																														if ($rate == '1') {
																																															echo $total_ot;
																																														}
																																														?>"></td>
															<td width="88" align="center"><?
																										...
																										?><input type="text" name="irate1_5[]" class="form-control" value="<?
																																														if ($rate == '1.5') {
																																															echo $total_ot;
																																														}
																																														?>"></td>
															<td width="60" align="center"><?
																										...
																										?><input type="text" name="irate2[]" class="form-control" value="<?
																																														if ($rate == '2') {
																																															echo $total_ot;
																																														}
																																														?>"></td>
															<td width="156" align="center"><?
																										...
																										?><input type="text" name="irate3[]" class="form-control" value="<?
																																														if ($rate == '3') {
																																															echo $total_ot;
																																														}
																																														?>"></td>
															<td align="center"><?
																							...
																							?><input type="text" name="ifl[]" class="form-control" value="<?= $sFL ?>"></td>
															<td align="center"><?
																							if ($sShift == 'Night') {
																								//echo "yes";
																								?><input type="text" name="ishiftval[]" class="form-control" value="yes"><?
																																														} else {
																																															?><input type="text" name="ishiftval[]" class="form-control" value=""><?
																																																																				}
																																																																				?></td>
															<td align="center"><? //$remark 
																							?><input type="text" name="iremark[]" class="form-control" value="<?= $remark ?>"></td>
														</tr>
												<?
														}
													}
									...
									if ($status == 'save') {
										...
										?><input type="button" value="Back" onClick="location='view_att_list2.php?empno=<?= $empno ?>&paycode=<?= $paycode ?>'"><?
