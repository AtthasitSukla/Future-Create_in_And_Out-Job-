    ...
    <script>
        ...
        function estimate_revenue_from_order() {
            ...
            var chart = new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [
                        ...
                        {
                            label: 'SAB KD',
                            backgroundColor: 'rgb(167, 249, 155)',
                            borderColor: 'rgb(167, 249, 155)',
                            fill: false,
                            data: []
                        },
                        {
            ...
            $.post("getajax_dashbord_sale.php", {
                ...
                var res_sab_kd = bb[1].split("#");
                var res_topre = bb[2].split("#");
                res_sab.forEach(function(entry) {
                    chart.data.datasets[0].data.push(entry);
                    chart.update();
                })
                res_sab_kd.forEach(function(entry) {
        ...
        function revenue_accumulated() {
            ...
            var chart = new Chart(ctx, {
                ...
                options: {
                    ...
                    title: {
                        display: true,
                        text: 'Revenue <?=$arr_month[$mmonth]?> <?=$yyear?>'
                    }
            ...
            var chart = new Chart(ctx, {
                ...
                data: {
                    ...
                    labels: [<?
                        ...
                        $select_wk = "SELECT nickname,sum(total_grand) as total_grand from View_summary_invoice_cn_create_date WHERE  MONTH(create_date) = $mmonth and YEAR(create_date)=$yyear
                        group by nickname
                        order by total_grand desc ";
                        $re_wk = mssql_query($select_wk);
                        while($row_wk = mssql_fetch_array($re_wk)){
            ...
            var chart = new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        (sum(total_grand)* 100 / (Select sum(total_grand) From View_summary_invoice_cn_create_date WHERE  MONTH(create_date) = $mmonth and YEAR(create_date)=$yyear)) as total_grand
                        from View_summary_invoice_cn_create_date WHERE  MONTH(create_date) = $mmonth and YEAR(create_date)=$yyear
                        group by nickname
                        order by total_grand desc ";
                        $re_wk = mssql_query($select_wk);
                        while($row_wk = mssql_fetch_array($re_wk)){
        ...
        function sab_order_type_packaging(){
            ...
            $sql_sab_rack = "SELECT COUNT(Part_No) AS count_Part_No, Part_No,SNEP 
            FROM View_saborder_package_type 
            WHERE UPPER(package_type)='RACK'
            AND Delivery_order_date_Month='$mmonth' 
            AND Delivery_order_date_year='$yyear' 
            GROUP BY Part_No,SNEP";
            $res_sab_rack = mssql_query($sql_sab_rack);
            while($row_sab_rack = mssql_fetch_array($res_sab_rack)){
                $count_Part_No = $row_sab_rack["count_Part_No"];
                $Part_No = $row_sab_rack["Part_No"];
                $SNEP = $row_sab_rack["SNEP"];
                ...
                $total_rack_price = $total_rack_price+($amount);
                include "connect_hq.php";
            ...
            include "connect_hq.php";
            $sql_sab_box = "SELECT COUNT(Part_No) AS count_Part_No, Part_No,SNEP 
            FROM View_saborder_package_type 
            WHERE UPPER(package_type)='BOX'
            AND Delivery_order_date_Month='$mmonth' 
            AND Delivery_order_date_year='$yyear' 
            GROUP BY Part_No,SNEP";
            $res_sab_box = mssql_query($sql_sab_box);
            while($row_sab_box = mssql_fetch_array($res_sab_box)){
                $count_Part_No = $row_sab_box["count_Part_No"];
                $Part_No = $row_sab_box["Part_No"];
                $SNEP = $row_sab_box["SNEP"];
                ...
                $total_box_price = $total_box_price+($amount);
                include "connect_hq.php";
            ...
            FROM View_saborder_package_type 
            WHERE UPPER(package_type)='MODULE'
            AND Delivery_order_date_Month='$mmonth' 
            AND Delivery_order_date_year='$yyear' 
            GROUP BY Part_No,SNEP";
            ...
            var data = google.visualization.arrayToDataTable([
                ['Task', 'Bath'],
                ['Rack', <?=round($total_rack_price,2)?>],
                ['Box', <?=round($total_box_price,2)?>],
        ...
        function topre_order_type_packaging(){
            ...
            var data = google.visualization.arrayToDataTable([
                ['Task', 'Bath'],
                ['Rack', <?=round($total_rackbox_price,2)?>],
                ['Box', 0],
                ['Module', <?=round($total_module_price,2)?>],
