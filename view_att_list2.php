...
$selectot="select id,shift,ot,fl,remark from tbot_mng where empno='".$empno."' and CAST(workdate AS DATE)='".$row0['workdate']."' order by id desc  ";
	...
	if($numot>0){
		...
		//check leave date
		$select_le = "select * from  tbleave_transaction where CAST(leavestartdate AS DATE)='".$row0['workdate']."' and empno='$empno'  ";
...
//echo $time;
	...
	//	echo ">>".DateTimeDiff($calc_rate,$attDate2." ".$startotday)."<<";
		...
		?></td>
        <td align="center"><? //$remark ?><input type="text" name="iremark[]" class="form-control" value="<?=$remark?>"></td>
      </tr>
      <?
	}
	}
