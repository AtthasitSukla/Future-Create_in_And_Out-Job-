...
if($status==''){
	...
	?>
       ...
       <td width="5%" ><button class="btn-primary btn" onClick="location='view_asset.php?asset_no='+document.getElementById('asset_no').value+'&asset_group='+document.getElementById('asset_group').value+'&device_type='+document.getElementById('device_type').value+'&location='+document.getElementById('location').value+'&asset_status='+document.getElementById('asset_status').value+''">ค้นหา</button></td>
       <td width="22%" >
		<?php
		if(!empty($location)){
			?>
	   ...
	   <td width="5%" ><button class="btn-primary btn" onClick="location='asset_viewer.php?status=add'">Add New</button></td>
       </tr>
	</table>
   <BR>
  ...
  <tr >
    ...
    <?
			...
			if($asset_no!=''){
				 ...
				 * from  tbasset   
				where asset_no like '%".$asset_no."%' OR serialnumber like '%".$asset_no."%'  ";
				if($location!=''){
				...
				if($asset_status!=''){
					$select3 = $select3."  and asset_status='$asset_status' ";
						...
						$select3 = $select3."  and asset_status in('normal_use') ";
						}
				
				}else{
					//and  location<>'OSW'
					$select3="select convert(varchar, po_open_date, 103)as po_open_date2,
				convert(varchar, purchase_date, 103)as purchase_date2,* from  tbasset where 1=1   ";
					if($location!=''){
					$select3 = $select3."  and location='$location'  ";
					...
					if($asset_group=='' && $device_type=='' && $asset_status==''){
						$select3 = $select3."  and asset_group not in('Warehouse Equipment','Software','Storage') ";
						}
					
				 
					$select3 = $select3."  order by asset_no asc ";
    	//echo $select3;
 ...
 //   echo   $sqlemp;
    ...
    <td width="12%" align="center"><?=$row3['location'];?></td>
      <td width="12%" align="center"><?=iconv("tis-620", "utf-8", $row3['asset_group'] );?><BR>
      PO:<?
      echo  $row3['po_open_date2'];
	  ?><BR>PC:<?
      echo  $row3['purchase_date2'];
	   ?><BR>Price:<?
	    echo  $row3['purchase_price'];
	   ?>
      </td>
...
//			}
      ...
      <td width="9%" align="center"><input type="button" value="View" onClick="location='asset_viewer.php?asset_no=<?=$row3['asset_no']?>'"></td>
