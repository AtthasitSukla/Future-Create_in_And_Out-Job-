...
function showpaycode(){
	...
	var tsite = $("#tsite").val();
	var empno = $("#empno").val();
	window.location = 'view_employee_att.php?paycode='+paycode+'&tsite='+tsite;
	}
...
function updateshift(id){
	...
	function createreport(){
		...
		var tsite = $("#tsite").val();
			if(paycode!=''){
					window.open('popreport.php?status=rawatt&paycode='+paycode+'&tsite='+tsite+'','width=842,height=600, scrollbar=yes');
				}else{
...
if($status==''){
  ...
  <td width="19%"><select id="paycode" class="form-control"  name="paycode"   style="width:200px;">
    ...
    <?
    	...
    	$select0="SELECT paycode from tbpaycode WHERE paycodeyear in('2019','2020','2021') order by startdate asc ";
    ...
    <td width="13%" height="40" align="left"><input type="button" value="Select" onClick="showpaycode();"></td>
    ...
    <table width="100%"  cellspacing="2" cellpadding="0" class="table table-striped table-bordered datatables" id="example">
      ...
      <tr>
        <td width="5%"  align="center">Item</td>
      	<td width="9%"  align="center">Emp No</td>
      	<td width="10%"  align="center">Emp Level</td>
        <td width="25%"  align="center">Emp Name</td>
        <td width="16%"  align="center">OT Management</td> 
        <!-- <td width="12%"  align="center">View Detail</td>  -->
        <td width="23%"  align="center">Status Approve</td>     
      </tr>
      <?
	  
	 
      	$select="select empno,emp_level from tbemployee where emp_level in('1','2','3') and site='$tsite' and delstatus<>'1'";
	//echo $select;
	$re=mssql_query($select);
	$num = mssql_num_rows($re);
	if($num>0){
       ...
       <tr>
        <td  align="center"><?=$i?>.</td>
		  ...
		  ?></td>
            ...
            <td  align="center"><a href="ot_mng.php?empno=<?=$row['empno']?>&paycode=<?=$paycode?>" target="_blank">Set OT</a></td>
            <!-- <td  align="center"><?
            if($_SESSION['emp_level']>=3 || $_SESSION['admin_userid']=='56038'){
			?><a href="view_att_list_new.php?empno=<?=$row['empno']?>&paycode=<?=$paycode?>" target="_blank">Approve OT</a><?
			}
			?></td> -->
              <td  align="center"><?
			  if($_SESSION['emp_level']>=3 || $_SESSION['admin_userid']=='56038'){
              $selectp="select paycode from  tbatt_approve where  empno='".$row['empno']."' and paycode='$paycode' ";
				$rep=mssql_query($selectp);
				$nump=mssql_num_rows($rep);
				if($nump>0){
					echo "Approved";
					?> [<a href="popatt.php?empno=<?=$row['empno']?>&paycode=<?=$paycode?>" target="_blank">Print OT</a>] [<a href="popatt_edit.php?empno=<?=$row['empno']?>&paycode=<?=$paycode?>" target="_blank">Edit OT</a>]<?
					}
			  }
			  ?></td>
